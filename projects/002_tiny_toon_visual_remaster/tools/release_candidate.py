...
def pack_fingerprint(pack_dir: Path) -> str:
    ...
    for path in [hires, *_referenced_images(pack_dir)]:
        ...
        digest.update((_sha256(path) if path.is_file() else "MISSING").encode("ascii")); digest.update(b"\0")
    return digest.hexdigest()
...
def default_regression_manifest() -> dict:
    return {
        ...
        "warning": "Final visual regression evidence is build-specific and becomes stale after runtime art/mapping changes.",
        "cases": {
            key: {"label": label, "done": False, "notes": "", "pack_fingerprint": None, "completed_utc": None}
...
def complete_regression_case(manifest_path: Path, case_key: str, pack_dir: Path, notes: str = "") -> dict:
    ...
    case.update(done=True, notes=notes, pack_fingerprint=fingerprint, completed_utc=datetime.now(timezone.utc).isoformat())
...
def regression_status(manifest_path: Path, pack_dir: Path) -> dict:
    ...
    for key, item in data["cases"].items():
        ...
        valid_done += int(is_current)
        stale += int(is_done and not is_current)
...
def _load_art_qa(path: Path, current_fingerprint: str) -> dict:
    ...
    result = {"exists": report.is_file(), "gate": "BLOCKED", "report_gate": None, "fingerprint_matches": False, "report_fingerprint": None, "path": str(report)}
...
def audit_release_candidate(
    ...
    if context["gate"] != "PASS":
        if not context["exists"]:
            ...
            blockers.append(f"Visual Context Review incomplete (pending={context['pending']}, stale={context['stale']})")
    if qa["gate"] != "PASS":
        if not qa["exists"]:
    ...
    if regression["gate"] != "PASS":
        blockers.append(f"Current-build full-game regression incomplete ({regression['done_current_build']}/{regression['total']}, stale={regression['stale_evidence']})")
    ...
    return {
        ...
        "important_note": (
            ...
            "a current Visual Context Review, current-build pixel QA and current-build full-game regression evidence."
        ),
        "pack": {"scale": stats.scale, "images": len(stats.images), "tile_rules": stats.tile_rules, "unique_tile_ids": stats.unique_tile_ids, "unique_palettes": stats.unique_palettes, "missing_images": list(stats.missing_images)},
...
def write_release_dashboard(result: dict, output_dir: Path) -> Path:
    ...
    regression_rows = "".join(
        "<tr>" + f"<td>{'PASS' if case['current_build'] else ('STALE' if case['done'] else 'TODO')}</td>" +
        f"<td><code>{html.escape(case['key'])}</code></td><td>{html.escape(case['label'])}</td><td>{html.escape(case.get('notes') or '')}</td></tr>"
...
<div class='card'><h2>Evidence summary</h2><p>Capture: <b>{result['capture']['done']}/{result['capture']['total']}</b> · Art queue: <b>{result['art_queue']['done']}/{result['art_queue']['rows']}</b> done · Visual context: <b>{context['gate']}</b> ({context['reviewed']}/{context['review_required']}) · Pixel QA: <b>{result['art_qa']['gate']}</b> · Regression: <b>{result['regression']['done_current_build']}/{result['regression']['total']}</b></p></div>
<div class='card'><h2>Final visual regression</h2><table><tr><th>Status</th><th>Case</th><th>Scope</th><th>Notes</th></tr>{regression_rows}</table></div></body></html>"""
    html_path.write_text(document, encoding="utf-8")
    return html_path


def audit_and_write(pack: Path, capture: Path, queue: Path, art_qa: Path, regression: Path, visual_review: Path, output: Path) -> dict:
...
def main() -> int:
    ...
    init = commands.add_parser("init-regression"); init.add_argument("manifest", type=Path)
    complete = commands.add_parser("complete-regression"); complete.add_argument("manifest", type=Path); complete.add_argument("case"); complete.add_argument("pack", type=Path); complete.add_argument("--notes", default="")
    for name in ("audit", "package"):
        ...
        if name == "package": cmd.add_argument("--zip", type=Path, required=True)
    args = parser.parse_args()
    if args.command == "init-regression":
        print(ensure_regression_manifest(args.manifest)); return 0
    if args.command == "complete-regression":
        print(json.dumps(complete_regression_case(args.manifest, args.case, args.pack, args.notes), indent=2)); return 0
    result = audit_and_write(args.pack, args.capture, args.queue, args.art_qa, args.regression, args.visual_review, args.output)
    print(json.dumps(result, indent=2))
    if result["release_gate"] != "PASS": return 2
    if args.command == "package": print(package_hd_pack(args.pack, args.zip))
    return 0
